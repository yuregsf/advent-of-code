...
function part1(input: string): number {
  ...
  for (const column of columns) {
    sum += Array.from(column.matchAll(/XMAS/g)).length
    sum += Array.from(column.split('').reverse().join('').matchAll(/XMAS/g)).length
  }

  for (const diagonal of diagonals) {
    sum += Array.from(diagonal.matchAll(/XMAS/g)).length
    sum += Array.from(diagonal.split('').reverse().join('').matchAll(/XMAS/g)).length
  }
  return sum;
}
